...
import { totpSetup, faceEnrollStart, faceEnrollFinish, faceStatus } from '../api/auth';
...
export default function Dashboard() {
  ...
  const handleEnrollFace = async () => {
    setMsg(''); setQrUrl('');
    try {
      await faceEnrollStart();
      setShowFaceCapture(true);
    } catch (e) {
      setMsgType('error');
      setMsg(e.response?.data?.error || e.message || 'Failed to start face enrollment.');
    }
  };
  ...
  const handleFaceCapture = async (capturedFaceData) => {
    ...
    try {
      await faceEnrollFinish(capturedFaceData);
      setFaceEnrolled(true);
      setMsgType('success');
      setMsg('✓ Face ID enrolled successfully! You can now login with your face.');
    } catch (e) {
      setMsgType('error');
      setMsg(e.response?.data?.error || e.message || 'Failed to enroll face.');
    }
  };
    ...
    <div style={d.page}>
      ...
      <div style={d.main}>
        ...
        <div style={d.methodGrid}>
          ...
          {/* Face ID */}
          <div
            style={{ ...d.methodCard, ...(hoveredCard === 'face' ? d.methodCardHover : {}) }}
            onMouseEnter={() => setHoveredCard('face')}
            onMouseLeave={() => setHoveredCard(null)}
          >
            <div style={d.methodTop}>
              <div style={{ ...d.methodIconCircle, background: 'rgba(59,130,246,0.12)', border: '1px solid rgba(59,130,246,0.25)' }}>
              ...
              <div style={{ ...d.methodBadge, background: faceEnrolled ? 'rgba(34,197,94,0.1)' : 'rgba(59,130,246,0.1)', color: faceEnrolled ? '#86efac' : '#93c5fd', border: `1px solid ${faceEnrolled ? 'rgba(34,197,94,0.2)' : 'rgba(59,130,246,0.2)'}` }}>
                {faceEnrolled ? '✓ Enrolled' : 'Advanced'}
              </div>
            </div>
            <h4 style={d.methodTitle}>Face ID</h4>
            <p style={d.methodSub}>Advanced Biometric Recognition</p>
            <p style={d.methodDesc}>
              Pixel-perfect face analysis with eye tracking and blink detection. Anti-spoofing technology built-in.
            </p>
            <div style={d.methodTags}>
              <span style={d.tag}>👁️ Eye Tracking</span>
              <span style={d.tag}>🔄 Liveness Check</span>
            </div>
            <button style={{ ...d.methodBtn, background: 'rgba(59,130,246,0.1)', borderColor: 'rgba(59,130,246,0.3)', color: '#93c5fd' }}
              onClick={handleEnrollFace}
              disabled={faceEnrolled}>
              {faceEnrolled ? '✓ Already Enrolled' : '👤 Enroll Face →'}
            </button>
          </div>
...
const d = {
  ...
  methodGrid:    { display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 20 },
