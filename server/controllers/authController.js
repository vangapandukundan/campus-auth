  ...
  try {
    ...
    const options = await generateRegistrationOptions({
      ...
      authenticatorSelection: {
        ...
        authenticatorAttachment: 'platform', // FORCE local fingerprint / Windows Hello
      },
    });
...
exports.webAuthnLoginStart = async (req, res) => {
  try {
    ...
    // Manually set allowCredentials with proper base64url string IDs
    // Strip 'hybrid' so Windows prefers the local fingerprint sensor over asking for a phone
    options.allowCredentials = user.credentials.map(c => ({
      id:         c.credentialID,
      type:       'public-key',
...
// ────────────────────────────────────────────────────────────────
const { enrollFace, verifyFace } = require('../utils/faceRecognition');

// ── FACE ENROLLMENT START ──────────────────────────────────────
// Frontend sends face frame with landmarks, blinks, head movement
exports.faceEnrollStart = async (req, res) => {
  try {
    const user = req.user;
    
    // Return success - frontend will capture face data in real-time
    res.json({
      message: 'Ready to enroll face. Please capture clear face image with eyes visible.',
      requirements: {
        duration: '5 seconds of face video',
        eyesVisible: 'Both eyes must be clearly visible',
        blinking: 'At least 3 natural blinks required',
        headMovement: 'Slight head movement needed (liveness check)',
...
// Frontend sends captured face data (landmarks, blinks, structure)
exports.faceEnrollFinish = async (req, res) => {
  try {
    const user = req.user;
    ...
    const biometrics = enrollFace(faceData);
    
    // Store in user record
    user.faceTemplate = biometrics.faceTemplate;
    user.eyePixelSignature = biometrics.eyePixelSignature;
    user.blinkPattern = biometrics.blinkPattern;
    user.faceStructure = biometrics.faceStructure;
    user.faceEnrolled = true;
    user.faceEnrollDate = new Date();
    await user.save();
    ...
    res.json({
      ...
      details: {
        template: 'Stored 128-dimensional face descriptor',
        iris: 'Eye signature captured (SHA256)',
        blinkRate: `${biometrics.blinkPattern} blinks/minute`,
        landmarks: 'Facial structure mapped (68 points)',
      }
    });
  } catch (err) {
    res.status(400).json({ error: err.message });
  ...
  try {
    ...
    if (!user.faceEnrolled) {
      return res.status(400).json({ 
        error: 'Face not enrolled. Please register your face first.',
        action: 'Use TOTP login instead' 
      });
    }
    
    res.json({
      message: 'Ready for face authentication. Look at camera.',
      requirements: {
        duration: '3-5 seconds',
        eyesVisible: 'Keep both eyes visible',
        blinking: 'Blink naturally (liveness check)',
        noMask: 'Remove any face coverings',
      }
    });
  } catch (err) {
    res.status(400).json({ error: err.message });
...
// ── FACE LOGIN FINISH ──────────────────────────────────────────
exports.faceLoginFinish = async (req, res) => {
  try {
    ...
    // Verify captured face against stored template
    const verification = verifyFace(faceData, {
      faceTemplate: user.faceTemplate,
      blinkPattern: user.blinkPattern,
      faceStructure: user.faceStructure,
    });
    ...
    if (!verification.isMatch || verification.confidence < 60) {
      return res.status(401).json({ 
        ...
        message: 'Your face did not match. Try again or use TOTP backup.',
      });
    }
    
    // Update last auth time
    user.faceLastAuth = new Date();
    await user.save();
    ...
    res.json({
      ...
      authentication: {
        ...
        confidence: verification.confidence,
        liveness: verification.livenessVerified,
      }
    });
  } catch (err) {
...
exports.faceStatus = async (req, res) => {
  try {
    ...
    res.json({
      ...
      enrollDate: user.faceEnrollDate,
      lastAuth: user.faceLastAuth,
      message: user.faceEnrolled 
        ? '✅ Face biometric is enrolled and ready'
        : '❌ Face not yet enrolled. Go to Dashboard to enroll.',
    });
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
